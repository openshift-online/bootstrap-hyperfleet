...
oc logs vault-helm-0 -n vault | grep "Unseal Key"
# Output: Unseal Key: /4k5SX8RvkEZYYXIDqqDKbPyV1cClnZpjMrzZ9lurYQ=

# Initialize and unseal (dev environment auto-unseals)
oc exec vault-helm-0 -n vault -- vault operator init -key-shares=1 -key-threshold=1
oc exec vault-helm-0 -n vault -- vault operator unseal /4k5SX8RvkEZYYXIDqqDKbPyV1cClnZpjMrzZ9lurYQ=
oc exec vault-helm-0 -n vault -- vault auth -method=token root
```
...
oc exec vault-helm-0 -n vault -- vault auth enable kubernetes
oc exec vault-helm-0 -n vault -- vault write auth/kubernetes/config \
...
echo 'path "secret/data/aws-credentials" { capabilities = ["read"] }
  path "secret/data/pull-secret" { capabilities = ["read"] }' | oc exec vault-helm-0 -n vault -i -- vault policy write cluster-secrets -

# Create Kubernetes role
oc exec vault-helm-0 -n vault -- vault write auth/kubernetes/role/cluster-role \
...
```bash
# AWS credentials
oc exec vault-helm-0 -n vault -- vault kv put secret/aws-credentials \
    aws_access_key_id="$(cat .secrets/aws.secret.id )" \
    aws_secret_access_key="$(cat .secrets/aws.secret.key )"

# Pull secret
oc exec vault-helm-0 -n vault -- vault kv put secret/pull-secret \
    .dockerconfigjson="$(cat .secrets/pull-secret.txt)"
    
# GitHub credentials
oc exec vault-helm-0 -n vault -- vault kv put secret/github-credentials \
    ssh-privatekey="$(cat .secrets/github.ssh.id_rsa)" \
    ssh-publickey="$(cat .secrets/github.ssh.id_rsa.pub)"
```
...
# Test authentication
oc exec vault-helm-0 -n vault -- vault write auth/kubernetes/login \
    role=cluster-role \
